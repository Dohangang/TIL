...
그리고 페인트를 다시 칠해야 할 구역들을 정했습니다. 벽에 페인트를 칠하는 롤러의 길이는 m미터이고, 롤러로 벽에 페인트를 한 번 칠하는 규칙은 다음과 같습니다.
...
function solution(n, m, section) {
  ...
  for (let i = 0; endSection <= section[section.length - 1]; i++) {
    if (section[i] < endSection) {
      continue;
    }
    endSection = section[i] + m;
    count++;
  }
